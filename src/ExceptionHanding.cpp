...
#define LAST_CIFF_ARRY_NUM (4)
    ...
    int EscapePlan::recordPitRol_10s(float val)
    {
        ...
        if (getTime() - record_time >= 1500)
        {
            ...
            {
                escActPara.escPre10PrePit = lastTimePitVal;
            ...
            {
                escActPara.escPre10PreRol = lastTimeRolVal;
            ...
            FRIZY_LOG(LOG_DEBUG, "record pit=%0.1f,rol=%0.1f[%0.1f,%0.1f]%0.1f,cnt=%d,%d", tmpPitEsc, tmpRolEsc, escActPara.escPre10PrePit, escActPara.escPre10PreRol, escActPara.escPre10PrePitRol, escActPara.escapPrePitChangCnt, escActPara.escapPreRolChangCnt);
            record_time = getTime();
        }
#if 0
        //每10s记录一次机器姿态
    ...
    void EscapePlan::escStepPreGyroAnal()
    {
        ...
        if (WHEELSTOP == chassisEscape.getWheelState())
        {
            if (++escPreStopTime > 10)
            {
                ...
                if (tmpEscPrePR > 3.5f || fabsf(getMEMSRelatAngle(0)) > 3.0f || fabsf(getMEMSRelatAngle(1)) > 3.0f)
                {
                ...
                else if (stuckType == ESCAPE_EVENT_TYPE_LIDAR_STUCK)
                {
                    ...
                    escActPara.escActStepRecord = ESC_STEP_LIDAR; //窗帘脱困
                    escActPara.enterStep0PreStep = ESCACT_START;
                }
            }
            else if (escPreStopTime == 5)
            {
                FRIZY_LOG(LOG_INFO, "sliding average:%f,before stick 10s:%f", escActPara.escStopPrePitRol, escActPara.escPre10PrePitRol);
            }
        }
        else
        {
    ...
    */
    int8_t EscapePlan::overheadCheck(float tmpDiff)
    {
        ...
        // 判定为架空脱成功了
        if (stuckType != ESCAPE_EVENT_TYPE_LEFTFLOOR_STUCK &&
            ((tmpDiff < 2.5f && fabsf(overPit) < 3.5f && fabsf(overRol) < 3.5f && actChangGraFlag) || (tmpDiff < 1.5f && fabsf(overPit) < 1.5f && fabsf(overRol) < 1.5f) || (actChangGraFlag && (fabsf(currPitTmp) < 3.2f && fabsf(currRolTmp) < 3.2f)) || (1 == escActPara.groundPeneDiff && actChangGraFlag)))
        {
            retTmp = 10;
            FRIZY_LOG(LOG_DEBUG, "overhead escape sucess!");
        }
        // else if(stuckType == ESCAPE_EVENT_TYPE_LEFTFLOOR_STUCK &&
        //           ((tmpDiff<2.5f && fabsf(overPit) <3.5f && fabsf(overRol) <3.5f && actChangGraFlag)
        ...
        //         ||(currPitTmp < 1.5f && currRolTmp < 1.5f))
        //         && addAngleError > 50)
        // {
        //     retTmp = 10;
    ...
    int EscapePlan::judgeActionStuckHead(float tmpDiff)
    {
        ...
        // if(act == 0)
        {
            ...
            FRIZY_LOG(LOG_DEBUG, "ActHead:%d %d %d,cnt r=%d b=%d c=%d L=%d in=%d\n", actPitRol, actPitRolTmp, escActPara.escActState, rightTryOverhCnt, behindTryOverhCnt, interTryOverhCnt, leftTryOverhCnt, frontTryOverhCnt);
        }
        else
        {
            if (actExeTime < 500)
    ...
    static int last_state_ = -1;
    static int behind_cnt_ = 0;
    static int dir_switch_ = 0;
    static int last_act_ = 0;
    int EscapePlan::judegActionLidar()
    {
        int rc = 0;
        ...
        if (behind_cnt_ > 10)
        {
            ...
            act = 32;
            escActPara.escActState = ESCACT_START;
            escActPara.escActStepRecord = ESC_STEP_FAILD; //切入成功
            return 0;
        }
        if (chassisEscape.getWheelState() == WHEELSTOP)
        {
            // FRIZY_LOG(LOG_DEBUG, "delay:%d", escActPara.actionRetDelay);
            if (last_state_ == ESCACT_BEHIND)
            {
                behind_cnt_++;
                if (beforeEscState)
                {
                    if (getAlongWallDir() == RIGHTAW)
                    {
                        escActPara.escActState = ESCACT_SPIN_RIGHT;
                    }
                    else
                    ...
                    last_state_ = escActPara.escActState;
                }
                else
                {
                    if (!last_act_)
            ...
            else if (last_state_ == ESCACT_SPIN_LEFT)
            {
                if (beforeEscState)
                {
                    escActPara.escActState = ESCACT_BEHIND;
            ...
            else if (last_state_ == ESCACT_SPIN_RIGHT)
            {
                if (beforeEscState)
                {
                    escActPara.escActState = ESCACT_BEHIND;
            ...
            // else if(last_state_ == ESCACT_NUILROT_LEFT)
            // {

            // }
            // else if(last_state_ == ESCACT_NUILROT_RIGHT)
            // {

            // }
            // else if(last_state_ == ESCACT_FRONT)
            // {
    ...
    void EscapePlan::lidarEscape()
    {
        switch (escActPara.escActState)
        {
            case ESCACT_START:
                ...
                dir_switch_ = 0;
                last_act_ = 0;
                FRIZY_LOG(LOG_DEBUG, "lidar init");
                escActPara.escActState = ESCACT_BEHIND;
                last_state_ = escActPara.escActState;
                break;
    ...
    void EscapePlan::escActStepSuccess()
    {
        ...
        case ESCACT_SUCCESS_PRE:
        {
            float relaTmpPit = getMEMSRelatAngle(0);
            float relaTmpRol = getMEMSRelatAngle(1);
            FRIZY_LOG(LOG_DEBUG, "relaTmpPit Rol:%f %f", relaTmpPit, relaTmpRol);

            if (relaTmpPit < 1.5f && relaTmpRol < 1.5f)
            {
                act = 32;
                escActPara.escActState = ESCACT_SUCCESS;
            }
            else if (relaTmpPit < -2.5f) //前面翘起
            {
                act = 13;
            }
            else if (relaTmpPit > 2.5f) //后面翘起
            {
                act = 12;
            }
            else if (relaTmpRol > 2.5f) //左边翘起
            {
                act = 18;
            }
            else //右边翘起
            ...
            escActPara.escActState = ESCACT_SUCCESS_PRE_WAIT;
        }
        break;

        case ESCACT_SUCCESS_PRE_WAIT:
        {
            if (chassisEscape.getWheelState() == WHEELSTOP)
            {
                FRIZY_LOG(LOG_DEBUG, "ESCACT_SUCCESS_PRE_WAIT 1");
                escActPara.escActState = ESCACT_SUCCESS;
            }
            else
            {
                if (getTime() - escActPara.escActExecuTime >= 3000)
                {
                    FRIZY_LOG(LOG_DEBUG, "ESCACT_SUCCESS_PRE_WAIT 2");
                    act = 32;
                    escActPara.escActState = ESCACT_SUCCESS;
                }
            }
        ...
        case ESCACT_END: //13脱困结束
        {
            ...
            if (-1 == escActPara.actionRetDelay || actOutDiff >= 5000) //自旋无效
            {
                act = 32;
    ...
    int EscapePlan::escDeal(int state)
    {
        ...
        escStartX = escapGrid.realx * 15 / 100;
        ; //记录脱困开始时的位姿
        ...
        while (GLOBAL_CONTROL == WHEEL_RUN)
        {
            ...
            if (getTime() - escStartTime >= 60000 * 2)
            {
                // escActPara.enterStep0PreStep = ESC_STEP_0;
                escActPara.escActStepRecord = ESC_STEP_FAILD;
    ...
    void EscapePlan::escAct()
    {
        while (1)
        {
            ...
            else
            {
                ...
                case 35:                                //雷达脱困
                    if (singleRotate(-100, -10, 50, 0)) //左单边旋 step4
                        act = 0;
                    break;

                case 36:
                    if (singleRotate(-10, -100, 50, 0)) //右单边旋 step4
                        act = 0;
                    break;

                case 37:
                    if (escSpin(100, 0, 90, 0)) //左自旋
                        act = 0;
                    break;

                case 38:
                    if (escSpin(100, 1, 90, 0)) //右自旋
                        act = 0;
                    break;

                case 39:
                    if (wheelCtrlStraight(100, 1200)) //前进
                        act = 0;
                    break;

                case 40:
                    if (wheelCtrlStraight(-100, 1200)) //后退
                        act = 0;
                    break;

                case 41:
                    if (escSpin(120, 0, 20, 0)) //左自旋
                        act = 0;
                    break;

                case 42:
                    if (escSpin(120, 1, 20, 0)) //右自旋
                        act = 0;
                    break;

                case 43:
                    if (escSpin(120, 0, 50, 0)) //左自旋
                        act = 0;
                    break;

                case 44:
                    if (escSpin(120, 1, 50, 0)) //右自旋
                        act = 0;
                    break;
                    // default:
                    //     act = 0;
    ...
    int EscapePlan::calcAimAglVelocity(void)
    {
        ...
        if (aimSpdR > WHEEL_TURN_MAX_SPD) //防止超出最大速度,计算出一个很大的无法到达的目标线速度
        {
            if (aimSpdR > 0)
            {
