...
namespace useerobot
{
            ...
            if ((searchKind == searchUnclean || searchKind == searchInside)
                ...
                if (all_path.empty()){
                    ...
                    if (searchKind == searchUnclean){
                        ...
                        Grid other = {aim.x+UporDown(cur,aim,1).redis,aim.y};
    ...
    bool UTurnPlanning::StopArch(Sensor sensor,Grid cur)
    {
        ...
        FRIZY_LOG(LOG_DEBUG,"MAX.%d.%d.%d.%d",_maze._map.xmax,_maze._map.xmin,_maze._map.ymax,_maze._map.ymin);

        if (archBound.up == 1000)
        {
            if (cur.x - _maze._map.xmin >= AREA_LENGTH 
            ...
            if (_maze._map.xmax - cur.x >= AREA_LENGTH 
        ...
        {
            if (cur.y - _maze._map.ymin >= AREA_LENGTH - 1 
            ...
            if (_maze._map.ymax - cur.y >= AREA_LENGTH - 1 
   ...
   int NearSeat(Grid cur,Grid aim){
        ...
        if (!_maze.seatPoint.empty()){
           ...
           if ((aim.forward > 300 || aim.forward < 60)
           ...
           if ((aim.forward > 30 && aim.forward < 150)
                && (fabs(aim.forward-cur.forward) < 5 || fabs(aim.forward-cur.forward) > 355)){
           ...
           if ((aim.forward > 120 && aim.forward < 240)
                && (fabs(aim.forward-cur.forward) < 5 || fabs(aim.forward-cur.forward) > 355)){
           ...
           if ((aim.forward > 210 && aim.forward < 330)
                && (fabs(aim.forward-cur.forward) < 5 || fabs(aim.forward-cur.forward) > 355)){
               if (_maze.GetMapState(cur.x,cur.y+1,1) == 4 || _maze.GetMapState(cur.x,cur.y+2,1) == 4){
