...
namespace useerobot
{
    ...
    void PathPlanningInterface::Init()
    {
        ...
        FRIZY_LOG(LOG_INFO,"init all 0330-1\n");
        ...
        case 5:
            {
                ...
                getBlockInfoIdenx = true;typedef enum LidarState{
                    NORMAL = 0, 
                    COVERED = 1, // 遮挡
                    WILD = 2, //空旷
                }lidat_state_t;
    ...
    void PathPlanningInterface::SLAMPathPLanning()
    {
        ...
        while (1)
        {
            ...
            // FRIZY_LOG(LOG_DEBUG, "gyro_stat_resp_state = %d , gyo_message_ok_index = %d,current_sensor.Initialized =%d",gyro_stat_resp_state,gyo_message_ok_index,current_sensor.Initialized); 
            
            // if(gyro_stat_resp_state !=true && current_sensor.Initialized != 1)
            // {
            ...
            FRIZY_LOG(LOG_DEBUG,"jinse: %d %d %d %d"
            ,road.fast_near_charge,current_sensor.Infrared_5253,current_sensor.Infrared,road.failCount);

            if(current_sensor.Infrared == 1 && !road.fast_near_charge)
            {
                record_charge_aim.x = current_pos.x;
                record_charge_aim.y = current_pos.y;
                FRIZY_LOG(LOG_INFO, "jilure : %d %d",record_charge_aim.x,record_charge_aim.y); 
            }
            ...
            // 正常回充
            if(road.call_recharge_index == true)
            { 
                robot_current_state.planningState = RECHARGER_REACHED; 
                handler->sendRobotPlanningState(robot_current_state); 
                slamPathPLanning_state_index = false;
    ...
    int PathPlanningInterface::SignManage(Sensor sensor,Grid cur)
    {
        ...
        if (!current_sensor.bump && (current_sensor.obs
            ...
            || (process == ROAD && roadState == roadFalse && NearSeat(current_pos,current_pos)))
            && roadState != roadTrue
            && IsWall() == 0 && lastLeft > 0 && lastRight > 0)
        {
            FRIZY_LOG(LOG_DEBUG,"obs2.%d  %d %d flr.%d %d %d",current_sensor.obs,_maze.Seat.x,_maze.Seat.y,
            current_sensor.radarObsState.frontTurn, current_sensor.radarObsState.leftTurn,
            current_sensor.radarObsState.rightTurn);
            obsStop = 1;
		}	
    ...
    void PathPlanningInterface::escapescene()
    {
        ...
        if(escape.escapeCheck())
        {
           ...
           _maze.RecordMap(tmp,current_pos);   
            ...
            int isSuccess = escape.escDeal(tmp_escap);
