...
{   extern bool cleanTaskOverIndex;
    ...
    void RoadPlanning::reCall(Grid cur,Grid center,int kind){
        ...
        if (_maze.GetMapState(cur.x+1,cur.y,2) == 2 && _maze.GetMapState(cur.x+1,cur.y,1) != 2
            ...
            tmpw = {cur.x+1,cur.y};
            if (find(workOb.begin(), workOb.end(),tmpw) == workOb.end()){
                FRIZY_LOG(LOG_DEBUG,"work : %d %d",tmpw.x,tmpw.y);
                workOb.push_back(tmpw);
            }      
        }
        ...
        if (((cur.x >= limit.up || cur.x <= limit.down || cur.y >= limit.left || cur.y <= limit.right)
            ...
            || find(CloseA.begin(), CloseA.end(),tmp) != CloseA.end()
            || CloseA.size() > 390 || workOb.size() > 390){

            FRIZY_LOG(LOG_DEBUG,"tuichu %d %d %d %d",cur.x,cur.y,CloseA.size(),workOb.size());
            return;
        }
            
        FRIZY_LOG(LOG_DEBUG,"cur : %d %d %d %d",cur.x,cur.y,CloseA.size(),workOb.size());

        CloseA.push_back(tmp);

        tmp = {cur.x+1,cur.y};
        reCall(tmp,center,kind); 
    ...
    void RoadPlanning::NullRunning(Sensor sensor,Grid cur)
    {
        ...
        if (road_aim.kind == searchBound || road_aim.kind == searchUnclean){
            ...
            if (!ggggg(road_aim.x,road_aim.y) && dewallSign
                ...
                FRIZY_LOG(LOG_DEBUG,"workOb: %d  %d",workOb.size() ,CloseA.size());
        ...
        else
        {
            ...
            if (road_aim.kind == recharge){
                ...
                if (cur.addAngle - start_add < -1000 || failCount >= 45){
            ...
            if (road_aim.kind == searchUnclean || road_aim.kind == searchBound){
                ...
                if (cur.addAngle - start_add < -1440){
                    FRIZY_LOG(LOG_DEBUG,"tiqianhuichong : %d",cur.addAngle - start_add);
                }
            }
            ...
            //two mode
            if (ggggg(road_aim.x,road_aim.y) == 1){
    ...
    void RoadPlanning::StartRoad(Sensor sensor,Grid cur,Trouble trouble)
    {
        ...
        case roadIdle:
            ...
            else if(road_aim.kind == appoint || road_aim.kind == zoning || road_aim.kind == home || road_aim.kind == breakClean){
                pathType = 2;
