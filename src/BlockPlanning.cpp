...
namespace useerobot
{
    ...
    int blockPlanning::SetInformation(Sensor sensor,Grid cur)
    {
        ...
        if (it == boundPoint.end()){
            ...
            if (keyIndex == -1){
                int tempi = it-boundPoint.begin();
                if (boundPoint.size() - tempi > 10 && tempi > 10
                    && boundPoint.back().addAngle - boundPoint[tempi].addAngle < -200){    
                    keyIndex = boundPoint.size()-1;
                    FRIZY_LOG(LOG_DEBUG,"yaozhu.%d  %d,%d",keyIndex,boundPoint[keyIndex].x,boundPoint[keyIndex].y);
                }
            }
        ...
        if (_maze._map.ymax - _maze._map.ymin >= AREA_LENGTH - 3 && limit.left == 1000)
        {
            if (_maze._map.ymax - _maze._map.ymin > AREA_LENGTH){
                ...
                limit.up = _maze._map.ymax;
                limit.down = _maze._map.ymin;
            }else{
    ...
    void blockPlanning::DivArea(Sensor sensor,Grid cur,Trouble trouble)
    {
        ...
        bool tmpS = abs(_maze._map.xmax - _maze._map.xmin) + abs(_maze._map.ymax - _maze._map.ymin) < 10 ? 
                    true:false;


        if ((FullBound(cur,trouble) == 1 && !tmpS)
            || continues > 6 || raoCount > 4 || mayFullbound.addAngle > 2
            || (tmpS && cur.addAngle - boundPoint[0].addAngle < -1440))
        {
            ...
            continues = 0;
            mayFullbound.addAngle = 0;
            _maze._map.xmax = -1000,_maze._map.xmin = 1000,_maze._map.ymax = -1000,_maze._map.ymin = 1000;
