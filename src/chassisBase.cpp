...
void test1(Grid cur){
    ...
    cloudPtr ptr = make_shared<PointCloud>(0);
    Point2f p;
    ...
    for (int i = 1;i < laserDis->size; ++i){  
        ...
        p.y = cur.realx*0.15 - dis*sin(aim_f);
    ...
    dynamic_map.resize(4*len*len,0);

    if (clouds_list.back()->index == 5){
        FRIZY_LOG(LOG_DEBUG,"set map");
        //删掉第一帧数据,插入第六帧数据
        ...
        dynamic_map[tmpI].count++;
        dynamic_map[0].robot_location.realx = clouds_list[5]->robot_location.realx;
        dynamic_map[0].robot_location.realy = clouds_list[5]->robot_location.realy;
        float dx;
        float dy;
        float de;
        Point2f p0;
        ...
        for (int i = 4; i >= 0;--i){
            ...
            dx = clouds_list[5]->robot_location.realx - clouds_list[i]->robot_location.realx;
            dy = clouds_list[5]->robot_location.realy - clouds_list[i]->robot_location.realy;
            de = clouds_list[5]->robot_location.forward - clouds_list[i]->robot_location.forward;
...
double chassisBase::Conlaser(){
    ...
    static radar_SlowTurn_t ObsCheck(){
        ...
        lastTime = getNowTime();
        // memset(&radarTmp,0,sizeof(radarTmp));
        FRIZY_LOG(LOG_DEBUG,"laserDis->size.%d",laserDis->size);
        if((leftCheckFlag & 1) == 1)
        {
            if(leftSlowCount)
            {
        ...
        for(int i = 0; i < laserDis->size; i ++)
        {
            ...
            if (laserDis->point[i].theta > ANGLE_TO_RADIAN(280) && laserDis->point[i].theta <= ANGLE_TO_RADIAN(330))
            {
                ...
                leftWallShort_R = 22.0f;
                leftWallLong_R = 35.0f;
                ...
                {  
                    ...
                    if(left_sum_laser_times > 3)
                    {
                        ...
                        leftWallShort_R = 19.0f;
                        leftWallLong_R = 26.0f;
                        if (leftTmp * 100 < ellipseCenterToSideDistance( (360 - (double)RADIAN_TO_ANGLE(laserDis->point[i].theta)) , leftWallShort_R , leftWallLong_R))
                        {
                            leftCheckFlag |= 2;
                            leftTurnCount = 1;
            ...
            if(laserDis->point[i].theta > ANGLE_TO_RADIAN(30) && laserDis->point[i].theta <= ANGLE_TO_RADIAN(80))
            {
                ...
                //     rightTmp = 0.0f;
                // }
                rightWallShort_R = 22.0f;
                rightWallLong_R = 35.0f;
                if (rightTmp * 100 < ellipseCenterToSideDistance( (double)RADIAN_TO_ANGLE(laserDis->point[i].theta) , rightWallShort_R , rightWallLong_R))
                {  
                    rightCheckFlag |= 1;
                    // FRIZY_LOG(LOG_DEBUG,"laser stop.%f,%f,%f",tmp,laserDis->point[i].x,laserDis->point[i].y);
                    ...
                    if(right_sum_laser_times > 3)
                    {
                        ...
                        rightWallShort_R = 19.0f;
                        rightWallLong_R = 26.0f;
                        if (rightTmp * 100 < ellipseCenterToSideDistance( (double)RADIAN_TO_ANGLE(laserDis->point[i].theta) , rightWallShort_R , rightWallLong_R))
                        {
                            rightCheckFlag |= 2;
                            rightTurnCount = 1;
...
void chassisBase::GridPoint(Grid* point)
{
        ...
        if (GLOBAL_CONTROL == WHEEL_RUN){
            ...
            if (abs(share_pose.x) > 200 || abs(share_pose.y) > 200){
...
{ 
        ...
        {
            FRIZY_LOG(LOG_DEBUG, "the huaqu is"); 
        ...
        {
            FRIZY_LOG(LOG_DEBUG, "the zhina is");
